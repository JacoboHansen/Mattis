...
export default function SafetyNotificationSettings({
  ...
  async function update(value: boolean) {
    ...
    try {
      ...
      setMessage(
        result.enabled ? 'Nøytrale oppfølgingsvarsler er på.' : 'Oppfølgingsvarsler er av.',
      );
    } catch (error) {
      setEnabled(previous);
      setMessage(error instanceof Error ? error.message : 'Innstillingen kunne ikke lagres.');
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <section className="card parent-settings-card safety-settings-card">
      <div>
        ...
        <span>
          Mattis kan sende en nøytral e-post hvis en elev skriver noe som tydelig bør følges opp.
          E-posten inneholder ikke sitater, detaljer eller diagnoser.
        </span>
      </div>
      ...
      <p className="field-hint">
        Dette er ikke en akuttjeneste. Ved umiddelbar fare: ring 113. Barn og unge kan kontakte
        Alarmtelefonen på 116 111.
      </p>
      {message ? (
