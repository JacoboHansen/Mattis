...
function publicSupabaseConfig() {
  ...
  if (!url || !key) throw new BillingConfigurationError('Supabase er ikke konfigurert.');
  return { url, key };
}
...
function serverSupabaseConfig() {
  ...
  if (!url || !key) {
    throw new BillingConfigurationError('Supabase sin servernøkkel er ikke konfigurert.');
  }
  return { url, key };
}
...
function stripeSecretKey() {
  ...
  if (!key) throw new BillingConfigurationError('Stripe sin servernøkkel er ikke konfigurert.');
  return key;
}

function appUrl() {
  const value = process.env.NEXT_PUBLIC_APP_URL ?? process.env.VERCEL_URL;
  if (!value) throw new BillingConfigurationError('Appens offentlige URL er ikke konfigurert.');
...
async function supabaseRequest<T>(
  ...
  const { url, key } = accessToken ? publicSupabaseConfig() : serverSupabaseConfig();
  ...
  if (!response.ok) throw new Error(`Supabase-feil (${response.status}): ${messageFromPayload(payload)}`);
  return payload as T;
}
...
export function toClientBillingStatus(account: BillingAccount | null): ClientBillingStatus {
...
export async function requireBillingAccess(accessToken: string, userId: string) {
  const account = await getBillingAccount(accessToken, userId);
  if (!isBillingEntitled(account)) throw new BillingAccessError();
  return account;
}

function encodeFormValue(value: unknown, prefix: string, form: URLSearchParams) {
  if (value === undefined || value === null) return;
  if (Array.isArray(value)) {
    value.forEach((item, index) => encodeFormValue(item, `${prefix}[${index}]`, form));
    return;
  }
...
function encodeForm(value: Record<string, unknown>) {
  ...
  Object.entries(value).forEach(([key, item]) => encodeFormValue(item, key, form));
  return form;
}
...
async function stripeRequest<T>(path: string, body: Record<string, unknown>) {
  ...
  const payload = await readJson(response);
  if (!response.ok) throw new Error(`Stripe-feil (${response.status}): ${messageFromPayload(payload)}`);
  return payload as T;
}
...
export async function retrieveStripeSubscription(subscriptionId: string) {
  return stripeGet<StripeObject>(`/subscriptions/${encodeURIComponent(subscriptionId)}`);
}
...
export async function syncExtraLearnerQuantity(
  ...
  await stripeRequest(`/subscriptions/${encodeURIComponent(account.stripe_subscription_id)}`, {
    items,
...
export async function saveBillingAccountAdmin(input: {
  ...
  if (input.stripeCustomerId !== undefined) body.stripe_customer_id = input.stripeCustomerId;
  if (input.stripeSubscriptionId !== undefined) {
    body.stripe_subscription_id = input.stripeSubscriptionId;
  }
  if (input.stripeBaseItemId !== undefined) body.stripe_base_item_id = input.stripeBaseItemId;
  if (input.stripeExtraItemId !== undefined) body.stripe_extra_item_id = input.stripeExtraItemId;
  if (input.currentPeriodEnd !== undefined) body.current_period_end = input.currentPeriodEnd;
...
export async function claimStripeEvent(id: string, type: string) {
  const payload = await supabaseRequest<StripeObject[]>('/rest/v1/stripe_webhook_events', {
    method: 'POST',
...
export async function releaseStripeEvent(id: string) {
  await supabaseRequest(`/rest/v1/stripe_webhook_events?id=eq.${encodeURIComponent(id)}`, {
    method: 'DELETE',
...
export function verifyStripeSignature(payload: string, header: string | null, secret: string) {
  ...
  if (!Number.isFinite(timestamp) || !signature || Math.abs(Date.now() / 1000 - timestamp) > 300) {
    return false;
  }
...
export function stripeWebhookSecret() {
  ...
  if (!secret) throw new BillingConfigurationError('Stripe webhook-secret er ikke konfigurert.');
  return secret;
}

export function stripeSubscriptionFields(subscription: StripeObject) {
  const items =
    subscription.items && typeof subscription.items === 'object' && !Array.isArray(subscription.items)
  ...
  return {
    status: (typeof subscription.status === 'string' ? subscription.status : 'inactive') as BillingStatus,
    stripeCustomerId:
