...
import {
  HomeworkParserError,
  parseHomeworkImages,
} from '../../apps/web/lib/ai/homework-parser';
...
describe('homework image parsing', () => {
  it('turns a vision response into validated, curriculum-tagged tasks', async () => {
    ...
    process.env.MATTIS_HOMEWORK_ENDPOINT =
    ...
    expect(requestBody.messages[0].content[2].image_url.url).toMatch(
    ...
    expect(requestBody.messages[0].content[0].text).toContain(
      'example: Et gjennomregnet eksempel',
    );
